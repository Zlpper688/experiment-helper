...
|---|---|---|
| 版本号 | manifest.json `0.7.0`（独立管理） | 头部 `@version 1.8.0`（独立管理，与扩展版**不同步是正常的**） |
...
- `content.js`：
  ...
  - `fetchReservedExperiments()` — fetch 当前 URL 解析「我的实验」页 → 已预约名单（实验间冲突、同类去重、AI/BI 进度都靠它）；**同时 fetch `action=scoreQuery` 成绩查询页** → 已做完实验名单（`doneExps`），用于行级 `isDone` 标记、隐藏已做同类、智能选课跳过。scoreQuery 失败为非致命错误，不影响主流程。
...
| 新人交接/首次使用 | `setScheduleInfo` 已内置四段确认：①`列表场次：第N周`（从当前列表行 `week` 取 max）②`已预约 N 个实验（自动同步）`（`fetchReservedExperiments` 每次进列表页自动跑）③`已做 N 个（自动同步）`（同函数内 fetch scoreQuery 页获取已做完实验）④课表导入状态。新用户看到 `已预约 0 个实验` 属正常，引导其先导入课表再选课 | 同步两版；fetch 失败时**不显示**对应数量以免误导，改显示同步失败提示；scoreQuery 失败为非致命，不影响已预约同步 |
| 已做实验自动同步 | `fetchReservedExperiments` 内部追加 fetch `action=scoreQuery` → 解析成绩查询页表格 → `doneExps` 数组 → 调用处 `doneNorms` Set → 行级 `isDone` 标记 + `lph-status-dup`「已做」徽标 | `isDone` 优先级低于 `isReserved`/`isSameType`（已预约的不会被误标为已做）；`hideDup` 复选框同时隐藏 `isSameType` 和 `isDone`；智能选课 `candidates` 过滤含 `!r.isDone` |
...
7. **已做实验同步验收**：在成绩查询页有记录的实验，进列表页后对应行显示「已做」徽标且默认被「隐藏已选/已做同类」隐藏；状态栏显示`已做 N 个（自动同步）`；取消勾选后行可见；一键抢课跳过已做实验
8. **验收底线**：不点预约按钮时，Network 面板不得出现任何 POST 请求

## 十、已知坑

- **乱码**：Windows PowerShell 控制台显示中文提交信息/文件会乱，实际存储是 UTF-8，别当成 bug 反复"修复"编码。
- **两套前缀**：`lph-`（扩展）与 `ulph-`（油猴）同时存在是有意的，防同页双装冲突，不要统一。
- **版本号**：油猴 `1.8.x` 与扩展 `0.7.x` 各自独立递增，不要互相"对齐"。
